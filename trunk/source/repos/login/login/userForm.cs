...
namespace login
{
    public partial class userForm : Form
    {
        ...
        private void registUser()
        {
            ...
            //DBへインサート,新規登録処理を行ったユーザーの情報をDBに登録
        ...
        protected void updateUser()
        {
            ...
            //DB接続し情報変更,変更処理を行ったユーザー情報をDBに登録
            DataAccess access = new DataAccess();
        ...
        private void update_login_user()
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("update admin.t_update_his                                 ");
            sb.AppendLine("set    update_login_user = :loginLastNm || :loginFirstNm  ");
            sb.AppendLine("where  updated_date =                                     ");
            sb.AppendLine("(select max(updated_date)                                 ");
            sb.AppendLine(" from   admin.t_update_his)                               ");
            String sql = sb.ToString();

            var paramList = new List<OracleParameter>();
                paramList.Add(new OracleParameter(":loginLastNm",
                                                  OracleDbType.NVarchar2,
                                                   LoginInfo.loginLastNm,
                                                   ParameterDirection.Input));
                paramList.Add(new OracleParameter(":loginFirstNm",
                                                   OracleDbType.NVarchar2,
                                                   LoginInfo.loginFirstNm,
                                                   ParameterDirection.Input));
            ...
            DataAccess access = new DataAccess();
            access.ExecuteSelect(sql, param);
        }
    }
}